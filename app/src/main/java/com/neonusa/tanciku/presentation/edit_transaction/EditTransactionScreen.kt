...
fun EditTransactionScreen(
    navigateUp: () -> Unit,
    event: (EditTransactionEvent) -> Unit,
    ...
    var rawAmount by remember { mutableStateOf(transaction.amount.toString()) } // for data input
    val initialFormattedAmount = NumberFormat.getInstance().format(rawAmount.toLongOrNull() ?: 0)

    val amount by remember{ mutableStateOf(initialFormattedAmount) } // for data view
    var desc by remember{ mutableStateOf(transaction.description) }
    var selectedDate by remember { mutableStateOf("") }

    var transactionType by remember { mutableStateOf(transaction.type.name) }
    var transactionCategory by remember { mutableStateOf(transaction.category.name) }

    var showDatePicker by remember { mutableStateOf(false) }
    var showSavingRecommendation by remember { mutableStateOf(true) }
...
fun EditTransactionScreenPreview(){
    EditTransactionScreen(navigateUp = {}, event = {}, allocation = Allocation(50,30,20), totalIncome = 5000000, totalSaving = 500000, transaction = Transaction(0,"","",TransactionType.Pengeluaran,TransactionCategory.Keinginan,0))
}